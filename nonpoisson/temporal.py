...
class CatalogVariability(object):
    ...
    def add(self, n, n2=None, ratio=None, dt=None, t1=None, t2=None, ti1=None,
            ti2=None):
    ...
    def plot_n2(self, ax=None, color='steelblue', start=0, end=np.inf,
                plot_points=True, alpha=0.05,
                markersize=0.1, linewidth=0.8, ylims=None, kernel_size=3,
                title=None):

        if ax is None:
            fig, ax = plt.subplots(1, 1)

        ind = np.argwhere(np.logical_and(np.array(self.n) > start,
                                         np.array(self.n) < end)).ravel()
        ...
        bound_max = medfilt(
            [np.quantile(ratio_n, 1 - alpha / 2.) for ratio_n in N2],
            kernel_size=kernel_size)
        bound_min = medfilt(
            [np.quantile(ratio_n, alpha / 2.) for ratio_n in N2],
            kernel_size=kernel_size)
    ...
    def plot_ratio(self, color='steelblue', start=0, end=np.inf,
                   plot_points=True, alpha=0.05,
                   markersize=0.1, linewidth=0.8, ylims=None, kernel_size=3,
                   low_cutoff=-2, title=None):
        ...
        Ratio = [[10 ** i if i != low_cutoff else 0 for i in ratio] for ratio
                 in Ratio]
        ...
        bound_max = medfilt(
            [np.quantile(ratio_n, 1 - alpha / 2.) for ratio_n in Ratio],
            kernel_size=kernel_size)
        bound_min = medfilt(
            [np.quantile(ratio_n, alpha / 2.) for ratio_n in Ratio],
            kernel_size=kernel_size)
    ...
    def plot_logratio(self, ax=None, color='steelblue', start=0, end=np.inf,
                      plot_points=True, alpha=0.05,
                      markersize=0.1, linewidth=0.8, ylims=(-2., 2.),
                      kernel_size=3, title=None):
    ...
    def plot_histogram(self, n, ax=None, bins=50, range_=None, label='Results',
                       ratio=False, color=None):
        ...
        ax.hist(Y, bins=bins, range=range_, alpha=0.3, label=label,
...
class CatalogAnalysis(object):
    ...
    def get_ratevar(self, verbose=True):
        ...
        for n in self.params['n_disc']:
            ...
            indices = random.sample(range(1, n_events - n),
                                    k=min(n_events - n - 1, max_iters))
            indices = np.sort(indices)

            t1 = np.stack((self.buffer(times[indices - 1], times[indices]),
                           self.buffer(times[indices + n - 1],
                                       times[indices + n]))).T
            dt = t1[:, 1] - t1[:, 0]

            ti1 = [times[(times > t1_k[0]) & (times <= t1_k[1])] for t1_k in
                   t1]
            t2 = np.stack((t1[:, 1], t1[:, 1] + dt)).T
            ti2 = [times[(times > t2_k[0]) & (times <= t2_k[1])] for t2_k in
                   t2]
            ...
            ratio = np.array(
                [np.log10(n2_i / n) if n2_i != 0 else ratio_cutoff for n2_i in
                 n2])
    ...
    def get_ratevar_time(self):
        ...
        for t in intervals_disc:
            ...
            for j in range(n_sub):
                t_intervals = np.arange(times.min() + t / (j + 1), times.max(),
                                        t)
                ...
                for n_i, n2_i, ratio_i in zip(n1, n2, ratio):
                    self.cat_var.add(n_i, n2_i, ratio_i,
                                     dt=np.diff(t_intervals)[0])
        ...
        for ind_i in range(n.shape[0]):
            n2.append(
                np.array(self.cat_var.n2)[np.argwhere(ind == ind_i)].ravel())
            ratio.append(np.array(self.cat_var.ratio)[
                             np.argwhere(ind == ind_i)].ravel())
        self.cat_var.n2 = n2
...
class Model(
    object):  # todo models are replacing themselves due to class inheritance
...
class forwardPoisson(Model):
    ...
    def simulate(self, nsims=None):
        ...
        for n1, params in self.params_local.items():
            ...
            ratio = np.array([np.log10(n2_i / n1) if n2_i != 0 else
                              self.metaparams['ratio_cutoff']
                              for n2_i in N2])
...
class NegBinom(Model):
    ...
    def plot_params(self, ax2=None):
        ...
        ax.set_ylabel(r'$\mu=\mathrm{Mean}(N_2 | N_1)$', color='brown', fontsize=14)

        return ax, ax2

    def extend_params(self, n_max, n_cut, n_min=1, limit=True,
                      parameters='nb2', deg=1, axes=None, plot=True):
        ...
        if parameters == 'nb2':
            ...
            poly_alpha = np.polyfit(np.log(n[id_learn]), alpha[id_learn],
                                    deg=2)
            ...
            if plot:
                if axes is None:
                    ...
                    ax2.plot(new_n, new_alpha, '.', color='blueviolet',
                             label=r'$\alpha$ fit')
                    ax2.set_ylabel(r'$\alpha$', color='blueviolet',
                                   fontsize=14)
                    ax2.set_xlabel(r'$N_1$', fontsize=14)
                    ax = ax2.twinx()
                    ax.plot(new_n, new_mu, '.', color='brown',
                            label='$\mu$ data')
                    ax.set_ylabel(r'Mean rate $\mu$', color='brown',
                                  fontsize=14)
                    axes = [ax, ax2]
                else:
                    axes[0].plot(new_n, new_mu, color='brown', linestyle='--',
                                 label='$\mu$ fit')
                    axes[1].plot(new_n, new_alpha, color='purple',
                                 linestyle='--', label=r'$\alpha$ fit')
            ...
            for i, n in enumerate(new_n):
                ...
                tau = 1. / self.params_local[n][3] * self.params_local[n][
                    2] ** Q
    ...
    def simulate(self, nsims=None):
        if not nsims:
            nsims = self.metaparams['sim_iters']
        for n1, params in self.params_local.items():
            n2 = [i for i in st.nbinom.rvs(params[0], params[1], size=nsims) if
                  i != 0]
            ratio = np.array([np.log10(n2_i / n1) if n2_i != 0 else
                              self.metaparams['ratio_cutoff']
                              for n2_i in n2])
    ...
    def get_conditional_numbers(self, n, nsims=1000):
        ...
        n_events = st.nbinom.rvs(self.params_local[n][0],
                                 self.params_local[n][1], size=nsims)
        return n_events
...
class MixedPoisson(Model):
    ...
    def get_params(self, catalog_analysis):
        ...
        self.metaparams['n_disc'] = catalog_analysis.params['n_disc']
        self.name += '_' + catalog_analysis.name if self.name == '%s-Poisson' % \
                                                    self.metaparams[
                                                        'distribution'] else ''
        ...
        for i, N in enumerate(catalog_analysis.cat_var.n):
            ...
            if self.metaparams['distribution'] in ('lognorm', 'gamma'):
                ...
                params = dist.fit(n2, floc=0, fscale=(np.max(
                    n2) + 1))  # 𝛼, 𝛽, loc (lower limit), scale (upper limit - lower limit)
    ...
    def plot_params(self, ax2=None, beta_params=(0, 1)):
        ...
        if self.metaparams['distribution'] == 'lognorm':
            ...
            ax.set_ylabel(r'$\mu_x=\mathrm{Med}[N]$', color='brown',
                          fontsize=14)
            ...
            p = np.array(
                [self.params_local[i][2] for i in n])  # theta = 1 / (p + 1)

            ax2.plot(n, tau, '.', color='blueviolet', label=r'$\tau data')
            ax2.set_ylabel(r'$\tau$', color='blueviolet', fontsize=14)
            ax2.set_xlabel(r'$N_1$', fontsize=14)
            ax = ax2.twinx()
            ax.plot(n, p, '.', color='brown',
                    label=r'$p=\frac{1-\theta}{\theta$ data')
            ...
            n = self.metaparams['n_disc']
            param_1 = np.array(
                [self.params_local[i][beta_params[0]] for i in n])
            param_2 = np.array([self.params_local[i][beta_params[1]] for i in
                                n])  # theta = 1 / (p + 1)

            ax2.plot(n, param_2, '.', color='blueviolet',
                     label=f'{param_names[beta_params[1]]} data')
            ax2.set_ylabel(f'{param_names[beta_params[1]]}',
                           color='blueviolet', fontsize=14)
            ax2.set_xlabel(r'$N_1$', fontsize=14)
            ax = ax2.twinx()
            ax.plot(n, param_1, '.', color='brown',
                    label=f'{param_names[beta_params[0]]} data')
            ax.set_ylabel(f'{param_names[beta_params[0]]}', color='brown',
                          fontsize=14)

        return ax, ax2

    def extend_params(self, n_max, n_cut, n_min=1, axes=None, plot=True,
                      beta_params=(0, 1)):
        ...
        if self.metaparams['distribution'] == 'lognorm':
            ...
            if plot:
                if axes is None:
                    fig, ax2 = plt.subplots()
                    ax2.plot(new_n, new_std, '-', color='blueviolet',
                             label=r'$\sigma_x$ fit')
                    ax2.set_ylabel(r'$\sigma_x', color='blueviolet',
                                   fontsize=14)
                    ax2.set_xlabel(r'N_1', fontsize=14)
                    ax = ax2.twinx()
                    ax.plot(new_n, new_mu, '-', color='brown',
                    ...
                    axes[0].plot(new_n, new_mu, color='brown', linestyle='--',
                                 label=r'$\xi$ fit')
                    axes[1].plot(new_n, new_std, color='purple',
                                 linestyle='--', label=r'$\sigma$ fit')
            ...
            poly_scale = np.polyfit(np.log(n[id_learn]), scale[id_learn],
                                    deg=3)
            ...
            if plot:
                param_names = [r'$\alpha$', r'$\beta$', r'location', r'scale']
                param_1 = np.array(
                    [self.params_local[i][beta_params[0]] for i in new_n])
                param_2 = np.array(
                    [self.params_local[i][beta_params[1]] for i in
                     new_n])  # theta = 1 / (p + 1)

                if axes is None:

                    fig, ax2 = plt.subplots()
                    ax2.plot(new_n, param_2, '.', color='blueviolet',
                             label=f'{param_names[beta_params[1]]} fit')
                    ax2.set_ylabel(f'{param_names[beta_params[1]]}',
                                   color='blueviolet', fontsize=14)
                    ax2.set_xlabel(r'n', fontsize=14)
                    ax = ax2.twinx()
                    ax.plot(new_n, param_1, '.', color='brown',
                            label=f'{param_names[beta_params[0]]} fit')
                    ax.set_ylabel(f'{param_names[beta_params[0]]}',
                                  color='brown', fontsize=14)
                    axes = (ax, ax2)

                else:
                    axes[0].plot(new_n, param_1, color='brown', linestyle='--',
                                 label=f'{param_names[beta_params[1]]} fit')
                    axes[1].plot(new_n, param_2, color='purple',
                                 linestyle='--',
                                 label=f'{param_names[beta_params[0]]} fit')
    ...
    def simulate(self, nsims=1000, mu_sims=None, number_sims=None):
        ...
        for n1, params in self.params_local.items():
            ...
            n2_full = np.array(
                [st.poisson.rvs(i, size=number_sims) for i in mu]).ravel()
    ...
    def get_conditional_numbers(self, n, nsims=1000):
        dist = getattr(st, self.metaparams['distribution'])
        mu_ln = dist.rvs(*self.params_local[n],
                         size=self.metaparams['sim_iters'])
        n_full = np.ravel(
            [st.poisson.rvs(i, size=int(0.4 * self.metaparams['sim_subiters']))
             for i in mu_ln])
...
class Empirical(Model):
    ...
    def simulate(self, nsims=None):
        if nsims is None:
            nsims = self.metaparams['sim_iters']
        for n1, params in self.params_local.items():
            distribution = st.rv_discrete(
                values=(np.arange(len(params)), params))
...
def create_models(cat):
    ...
    gnegbinom = GlobalNegBinom(
        metaparams={'sim_iters': 1000, 'loglike_method': 'nb1'})
