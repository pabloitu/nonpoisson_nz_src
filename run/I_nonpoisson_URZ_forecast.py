...
def create_forecast_ln(folder):

    temporal = handler_temporal.Model.load('lognorm-Poisson_nz_1960')
    ax=temporal.plot_params()
    temporal.extend_params(1000, 600, 10, plot=True, axes=ax)
    plt.tight_layout()
    plt.savefig(join(paths.fig_temporal_forecast, 'ln_params.png'), dpi=300, facecolor=(0, 0, 0, 0))
    plt.show()
